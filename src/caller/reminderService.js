...
class ReminderService {
  async collectAppointments(hours) {
    ...
    for (const appointment of appointmentsToCall) {
      //получение даты (вдруг сегодня или завтра прием)
      const time_start =
        (currentDate.toISOString().split("T")[0] ===
        appointment.time_from.toISOString().split("T")[0]
          ? "сегодня "
          : "завтра ") +
      ...
      axios
        ...
        .then((response) => {
          ...
          Object.keys(rawData).forEach((key) => {
            ...
            //есть data есть (т.е. где звонок завершился и нет errors: звонка нет)
            if (!("errors" in data)) {
              //получение title (title может быть пустым, если цель не определена поэтому не определено)
